...
class CodingAssistant {
  ...
  loadCodeIntoEditor(code) {
    ...
    if (embeddedToolCode) {
      ...
      // Ensure the editor is editable (Monaco or contenteditable)
      if (window.monacoEditorIntegration) {
        window.monacoEditorIntegration.ensureEditable('embedded-tool-code');
        
        // Try to set content via Monaco Editor first
        const success = window.monacoEditorIntegration.setContent('embedded-tool-code', code);
        if (success) {
          console.log('✅ Code loaded into Monaco Editor');
          return true;
        }
      }
      
      // Fallback to contenteditable - ensure it's editable first
      if (!embeddedToolCode.hasAttribute('contenteditable')) {
        embeddedToolCode.setAttribute('contenteditable', 'true');
      }
      
      const htmlContent = code.replace(/\n/g, '<br>');
      embeddedToolCode.innerHTML = htmlContent;
      embeddedToolCode.focus();
      console.log('✅ Code loaded into contenteditable editor');
      return true;
    }
    ...
    // Ensure the editor is editable (Monaco or contenteditable)
    if (window.monacoEditorIntegration && sourceEditor.id) {
      window.monacoEditorIntegration.ensureEditable(sourceEditor.id);
      
      // Try to set content via Monaco Editor first
      const success = window.monacoEditorIntegration.setContent(sourceEditor.id, code);
      if (success) {
        console.log('✅ Code loaded into Monaco Editor');
    ...
    // Fallback to contenteditable - ensure it's editable first
    if (!sourceEditor.hasAttribute('contenteditable')) {
    ...
    if (window.operatorsModule && window.operatorsModule.showToolEditor) {
      ...
      setTimeout(() => {
        ...
        if (toolNameInput && toolDescInput && codeEditor) {
          ...
          // Ensure the editor is editable and try to use Monaco Editor if available
          if (window.monacoEditorIntegration) {
            window.monacoEditorIntegration.ensureEditable('embedded-tool-code');
            
            const success = window.monacoEditorIntegration.setContent('embedded-tool-code', code);
            if (success) {
              console.log('✅ Code loaded into Monaco Editor for tool creation');
            } else {
              // Fallback to contenteditable - ensure it's editable
              if (!codeEditor.hasAttribute('contenteditable')) {
                codeEditor.setAttribute('contenteditable', 'true');
              }
              codeEditor.innerHTML = code.replace(/\n/g, '<br>');
              console.log('✅ Code loaded into contenteditable editor for tool creation');
            }
          } else {
            // Fallback to contenteditable - ensure it's editable
            if (!codeEditor.hasAttribute('contenteditable')) {
